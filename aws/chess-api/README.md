...
```bash
chess-api$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
chess-api$ sam local start-api
chess-api$ curl http://localhost:3000/classify_digit
```
...
```bash
chess-api$ sam logs -n InferenceFunction --stack-name "chess-api" --tail
```
...
```bash
sam delete --stack-name "chess-api"
```
